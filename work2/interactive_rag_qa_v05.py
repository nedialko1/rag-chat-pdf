...
# Switching back to gemma:2b now that we know its size is correct (1.7 GB) and hopefully the crash was transient.
llm = OllamaLLM(model="gemma:2b")

# 2. Define Prompt Template
# This prompt forces the model to synthesize a detailed, non-minimal answer
prompt = ChatPromptTemplate.from_template("""
    You are an expert data extractor. Answer the user's question by providing a 
    detailed and comprehensive answer based *only* on the provided context. 
    If the question asks about a person, include their professional title and company.
    If you cannot find the required information in the context, clearly state that the information is not available in the document.
...
# --- LLM Cold Start Warm-up (Performing a FULL RAG run) ---
print("Warming up Ollama model and RAG chain (this may take time on the first run)...")
# Run a dummy query through the ENTIRE chain to force all LLM and Chroma components to load.
DUMMY_QUERY = "Summarize the contact details found in the document."
try:
    start_time = time.time()
    # Execute a full retrieval and generation chain call
    warmup_response = retrieval_chain.invoke({"input": DUMMY_QUERY})
    end_time = time.time()

    # We ignore the answer, we only care about the load time
    print(f"Ollama/RAG warm-up complete in {end_time - start_time:.2f} seconds. System is ready.")
except Exception as e:
    # Updated warning to reflect the new model
    print(f"WARNING: Could not complete LLM warm-up. Ensure Ollama is running and 'gemma:2b' is pulled. Error: {e}")
    # We continue, but the first query will be slow

...
# Softened the prompt to allow for synthesis while remaining grounded.
...
# --- LLM Cold Start Warm-up ---
print("Warming up Ollama model (this may take 30-60 seconds on the first run)...")
# Note: Use a slightly more complex prompt and add a pause for robust model loading.
try:
    warmup_response = llm.invoke("You are a helpful assistant. Respond with the single word 'ready'.")
    print(f"Ollama warm-up complete. Response from LLM: {warmup_response.strip()}")
    time.sleep(1)  # Small pause to let resources settle after heavy I/O
except Exception as e:
